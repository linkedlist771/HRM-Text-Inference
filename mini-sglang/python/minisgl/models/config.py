...
class ModelConfig:
    ...
    # HRM-specific (defaults make non-HRM models unaffected)
    H_cycles: int = 1
    L_cycles: int = 1
    num_layers_per_stack: int = 0
    ...
    def from_hf(cls, config: PretrainedConfig) -> ModelConfig:
        ...
        return cls(
            ...
            H_cycles=getattr(config, "H_cycles", 1) or 1,
            L_cycles=getattr(config, "L_cycles", 1) or 1,
            num_layers_per_stack=getattr(config, "num_layers_per_stack", 0) or 0,
            embedding_scale=getattr(config, "embedding_scale", 1.0) or 1.0,
            prefix_lm=getattr(config, "prefix_lm", False) or False,
        )