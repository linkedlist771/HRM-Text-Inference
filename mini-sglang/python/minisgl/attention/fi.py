    ...
    def _initialize_metadata_once(self, metadata: FIMetadata) -> None:
        ...
        if isinstance(metadata.wrapper, BatchDecodeWithPagedKVCacheWrapper):
            ...
            metadata.wrapper.plan(
                ...
                kv_data_type=metadata.dtype,
                non_blocking=True,
                # HRM PrefixLM: the whole prompt is one bidirectional prefix block.
                # token_type_ids=all-ones in the reference == non-causal prefill here.
                causal=not getattr(self.config, "prefix_lm", False),
            )
        self.last_event.record()
